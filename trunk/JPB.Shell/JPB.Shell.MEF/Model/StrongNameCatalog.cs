...
namespace JPB.Shell.MEF.Model
{
    internal class StrongNameCatalog : ComposablePartCatalog, INotifyComposablePartCatalogChanged
    {
        ...
        /// <param name="withCheckForDuplicates"></param>
        /// <param name="trustedKeys">All Public keys that the application should trust</param>
        public StrongNameCatalog(IEnumerable<string> path, bool watchDirectorys, bool withCheckForDuplicates, params byte[][] trustedKeys)
        {
            WithCheckForDuplicates = withCheckForDuplicates;
            _paths = path;
            _watchDirectorys = watchDirectorys;
            _trustedKeys = trustedKeys;
            LowPriorityList = new ConcurrentQueue<string>();
            WaitForCompletition = new AutoResetEvent(false);
        }

        internal AutoResetEvent WaitForCompletition;

        public string PriorityKey { get; set; }
        public ConcurrentQueue<string> LowPriorityList { get; set; }
        public bool IgnoreAllNonPkAsseamblys { get; set; }
        ...
        public void AsyncInit()
        {
            ...
            foreach (string internalpath in _paths.Where(Directory.Exists))
            {
                ...
                var thread = new Thread(() =>
                {
                    ...
                    WaitForCompletition.Set();
                    WaitForCompletition.Dispose();
                    WaitForCompletition = null;
                });
